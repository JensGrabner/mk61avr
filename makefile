...
CPPFLAGS += -Wa,-adhlns=$(<:%.cpp=$(CURDIR)/%.lst)
...
ASFLAGS = $(ADEFS) -Wa,-adhlns=$(<:%.S=$(CURDIR)/%.lst),-gstabs,--listing-cont-lines=100
...
OBJ = $(SRC:%.c=$(CURDIR)/%.o) $(CPPSRC:%.cpp=$(CURDIR)/%.o) $(ASRC:%.S=$(CURDIR)/%.o)

# Define all listing files.
LST = $(SRC:%.c=$(CURDIR)/%.lst) $(CPPSRC:%.cpp=$(CURDIR)/%.lst) $(ASRC:%.S=$(CURDIR)/%.lst)
...
clean_list :
	...
	$(REMOVE) $(TARGET).sym
	$(REMOVE) $(TARGET).lss
	$(REMOVE) $(SRC:%.c=$(CURDIR)/%.o)
	$(REMOVE) $(SRC:%.c=$(CURDIR)/%.lst)
	$(REMOVE) $(SRC:.c=.s)
	$(REMOVE) $(SRC:.c=.d)
	$(REMOVE) $(SRC:.c=.i)
	$(REMOVEDIR) .dep
